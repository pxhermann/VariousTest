...
namespace Win32Mapi
{
	...
	#region SENDING
		...
		public bool Send(IntPtr sess, IntPtr hwnd, string subject, string body, int flag)
		{
			DeallocMessage();
			...
			error = MAPISendMail(sess, hwnd, lastMsg, flag, 0 );
	...
	#region READING
		...
		private bool SaveAttachByName( string name, string savepath )
		{
			...
			for( int i = 0; i < lastMsg.fileCount; i++ )
			{
				...
				catch( Exception ) { f = false; error = 13; }

				try { File.Delete( fdtmp.path );}  catch( Exception ) {}
			}
			return f;
		}
	#endregion
